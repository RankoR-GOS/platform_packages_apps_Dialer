...
android {
    ...
    testOptions {
        unitTests {
            // Robolectric loads classes through its own sandbox, and those carry no source
            // location. Without this JaCoCo silently skips every Robolectric-exercised class and
            // reports 0% for code that is thoroughly tested.
            all { unitTest ->
...
// --- test coverage ----------------------------------------------------------------------------
//
// Kept here rather than in an applied jacoco.gradle.kts: Android Lint crashes analysing a script
// applied that way (SymbolLightClassForScript -> addPropertyBackingFields), and lint is a gate on
// this project.
//
// Only the migrated packages are measured. Averaging them against 1100+ untested AOSP Java files
// would produce a number no threshold could be set against.

fun measuredClasses(roots: List<String>): List<ConfigurableFileTree> =
    roots.map { root ->
        fileTree(root) {
            include(
                "com/android/dialer/data/**",
                "com/android/dialer/domain/**",
                "com/android/dialer/di/**",
                "com/android/dialer/ui/**",
            )
            exclude(
                // Generated by Dagger, Hilt and KSP.
                "**/Dagger*",
                "**/Hilt_*",
                "**/*_Factory*",
                "**/*_MembersInjector*",
                ...
                "**/*_GeneratedInjector*",
                "**/*_Impl*",
                // Compose lifts every literal composable into a singleton holder.
                "**/ComposableSingletons*",
                // Abstract @Binds declarations; Dagger writes the implementation.
                "**/*BindsModule*",
                // Debug-only host for instrumented fragment tests.
                "**/ContactsHostFragmentTestActivity*",
            )
        }
    }

fun JacocoReportBase.coverageOf(roots: List<String>, executionPattern: String) {
    sourceDirectories.setFrom(files("../java"))
    classDirectories.setFrom(files(measuredClasses(roots)))
    executionData.setFrom(fileTree("build") { include(executionPattern) })
}

// A threshold only applies when its property is passed, so the reports stay usable for looking at
// where coverage actually is before anyone commits to a number.
fun coverageMinimum(propertyName: String): BigDecimal {
    val raw = project.findProperty(propertyName)?.toString() ?: return BigDecimal.ZERO
    val percent = raw.toBigDecimalOrNull()

    check(percent != null) { "-P$propertyName must be numeric, got: $raw" }
    check(percent in BigDecimal.ZERO..BigDecimal(100)) { "-P$propertyName must be 0..100" }

    return percent.divide(BigDecimal(100))
}

fun JacocoCoverageVerification.coverageGate(instructionProperty: String, branchProperty: String) {
    violationRules {
        rule {
            limit {
                counter = "INSTRUCTION"
                minimum = coverageMinimum(instructionProperty)
            }
            limit {
                counter = "BRANCH"
                minimum = coverageMinimum(branchProperty)
            }
        }
    }
}

val unitClassRoots = listOf(
    "build/intermediates/built_in_kotlinc/debug/compileDebugKotlin/classes",
    "build/intermediates/javac/debug/compileDebugJavaWithJavac/classes",
)
val instrumentedClassRoots =
    listOf("build/intermediates/classes/debug/transformDebugClassesWithAsm/dirs")
val unitExecutionData = "outputs/unit_test_code_coverage/debugUnitTest/*.exec"
val instrumentedExecutionData = "outputs/code_coverage/debugAndroidTest/connected/**/*.ec"

tasks.register<JacocoReport>("jacocoUnitTestReport") {
    group = "Reporting"
    description = "Coverage of the migrated packages from unit tests."
    dependsOn("testDebugUnitTest")
    coverageOf(roots = unitClassRoots, executionPattern = unitExecutionData)
    reports {
...
tasks.register<JacocoCoverageVerification>("jacocoUnitTestVerification") {
    group = "Verification"
    description = "Fails when unit test coverage of the migrated packages drops."
    dependsOn("jacocoUnitTestReport")
    coverageOf(roots = unitClassRoots, executionPattern = unitExecutionData)
    coverageGate(
        instructionProperty = "unitTestMinCoverage",
        branchProperty = "unitTestMinBranchCoverage",
    )
}

tasks.register<JacocoReport>("jacocoAndroidTestReport") {
    group = "Reporting"
    description = "Coverage of the migrated packages from instrumented tests."
    // The instrumented classes come from the ASM transform, and the execution data from the
    // connected test run. Ordering rather than dependsOn for the latter, so the report can be
    // regenerated from existing execution data without a device attached.
    dependsOn("transformDebugClassesWithAsm")
    mustRunAfter("connectedDebugAndroidTest")
    coverageOf(roots = instrumentedClassRoots, executionPattern = instrumentedExecutionData)
    reports {
        html.required.set(true)
        xml.required.set(true)
    }
...
tasks.register<JacocoCoverageVerification>("jacocoAndroidTestVerification") {
    group = "Verification"
    description = "Fails when instrumented test coverage of the migrated packages drops."
    dependsOn("jacocoAndroidTestReport")
    coverageOf(roots = instrumentedClassRoots, executionPattern = instrumentedExecutionData)
    coverageGate(
        instructionProperty = "androidTestMinCoverage",
        branchProperty = "androidTestMinBranchCoverage",
    )
}
